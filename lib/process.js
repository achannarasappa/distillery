...
var Process = function(definition, options){
  ...
  if (validateDefinition(definition)) {
    ...
    this.method = this.definition.request.method.toLowerCase();
  }
};
...
Process.prototype._buildConfiguration = function(parameters){
  return {
    ...
    url: Utility.interpolate(this.definition.request.url, generateParameters(this.definition.request.query, parameters)),
    headers: generateParameters(this.definition.request.headers, parameters),
    form: generateParameters(this.definition.request.payload, parameters)
  }
};

Process.prototype._generateResponse =  function(response){

  var valid_response_key = getValidResponseKey(this.definition.response, response);

  if (_.isUndefined(valid_response_key))
    return requestError('No response conditions met.', response);
  else
    return _.assign(response, {
      indicators: getValidResponseIndicators(this.definition.response[valid_response_key].indicators, response),
      hook: this.definition.response[valid_response_key].hook
    });
...
var getValidResponseKey = function(definitions, response) {
  return _.findKey(definitions, function(definition){
    return isResponseValid(definition, response);
  })
};

var getValidResponseIndicators = function(indicators, response) {
...
var requestError = function(message, response){
  return {
    message: message,
