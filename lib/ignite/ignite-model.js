...
Model.prototype.parse = function(html){
  ...
  if (definition.type === 'collection') {
    ...
    if (self.options.show_table) {
      ...
      _.map(_.first(iteration, self.options.table_rows), function($){
        if (_.isNull(self._applyFilters(definition.validate, definition.format, self._parseItem(definition.elements, $)))){
          table.push(_.map(_.values(self._parseItem(definition.elements, $)), _.compose(chalk.gray, Utility.truncateString(self.options.string_truncate_length))))
        } else {
          table.push(_.map(_.values(self._parseItem(definition.elements, $)), _.compose(chalk.white, Utility.truncateString(self.options.string_truncate_length))))
        }
      });
